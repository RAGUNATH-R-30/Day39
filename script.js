...
app.put("/assignmultiplestudents",async(req,res)=>{
  try {
    ...
    const mentor=await mentorcollection.findOne({_id:mentor_id})
    let studentIds = req.body.students.map((studentId)=>new ObjectId(studentId))
    
    for (const studentId of studentIds){

      const student = await collection.findOne({_id:studentId})
      const previous_mentor = student.mentor
      await collection.updateOne({_id:studentId},{$set:{
        previous_mentors:previous_mentor,
        mentor:{"_id":mentor._id,"name":mentor.name,"age":mentor.age,"Expertise":mentor.expertise}}}
      )
      await mentorcollection.updateOne({_id:mentor_id},{$push:{students:student}})
    }
    await connection.close();
    ...
    //   let student_id = new ObjectId(studentid)
